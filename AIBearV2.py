...
red_light_image = Image.open("red-light.png").resize((36, 53), Image.LANCZOS)
green_light_image = Image.open("green-light.png").resize((36, 53), Image.LANCZOS)
...
#gaze_feedback_label = tk.Label(window, font=("Helvetica", 24, "bold"))
#gaze_feedback_label.place(relx=0.5, rely=0.1, anchor="center")
...
def handle_conversation():
    global eye_contact_duration, total_duration, gaze_tracking_active

    # Stop current video and play transition video
    stop_video()
    play_video("bear_to_listening.mp4", loop=False)
    time.sleep(2)  # Ensure the video switches to bear_listening.mp4 before listening
    play_video("bear_is_listening.mp4", loop=True)  # Play listening loop
    ...
    if user_input:
        ...
        play_video("bear_stop_listening.mp4", loop=False)
        time.sleep(2)  # Ensure the video switches to bear_stop_listening.mp4
        play_video("bear_speaking.mp4", loop=True)  # Play speaking loop
        speak_text(response)
    stop_video()
    play_video("bear_idle.mp4", loop=True)
...
def gaze_tracking():
    ...
    while True:
        ...
        if gaze_tracking_active:
            if gaze.looking_at_centre():
                eye_contact_duration += 1 / 30  # assuming 30 FPS
                #gaze_feedback_label.config(text="YES", fg="green")
                print("Looking at centre")
                update_traffic_light('contact')
            else:
                #gaze_feedback_label.config(text="NO", fg="red")
...
# Import speak button
button_image = Image.open("button_speak.png")
button_photo = ImageTk.PhotoImage(button_image)

# Import stop button
stop_image = Image.open("button_stop.png")
...
save_button_image = Image.open("save-data.png")
